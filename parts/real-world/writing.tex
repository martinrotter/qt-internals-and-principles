...
Wrapper of muParserX is called \enquote{Calculator} and is represented by the class with the same name. Calculator offers method\fdocinlinecode{cpp}{!}{void calculateExpression(Calculator::CallerFunction function, QString expression)} which takes mathematical expression in textual form and identification of target function. This method is used by calculator and unit converter as these entities may want to do computations.

\indent \fdocinlinecode{cpp}{!}{Calculator} class will be used as \textit{singleton} pattern and will be separated into its own thread by\fdocinlinecode{cpp}{!}{CalculatorWrapper} class which manages\fdocinlinecode{cpp}{!}{Calculator} singleton instance. This class basically just starts the thread with calculator and quits the thread if needed (\autoref{listing:thrq}).
...
Qonverter style sheet can (but doesn't have to) take advantage of calculator buttons resolution and highlight each group of buttons differently. That's what Modern skin does. Modern skin file can be found in\fdocinlinecode{text}{!}{resources/skins/base} subdirectory of Qonverter source tree. This skin contains many enhancements for user interface plus distinctive coloring (\autoref{listing:stylebtn}) for calculator buttons.
